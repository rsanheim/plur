...
func TestDryRunString(t *testing.T) {
	t.Run("includes both plur env vars", func(t *testing.T) {
	...
	t.Run("serial mode only has PARALLEL_TEST_GROUPS", func(t *testing.T) {
		...
		cmd.Env = []string{
			...
			// No TEST_ENV_NUMBER in serial mode
		}
	...
	t.Run("env vars appear before command", func(t *testing.T) {
		...
		// Env vars should come first, then command
		// The exact order of env vars depends on iteration order
		assert.True(t,
	...
	t.Run("handles similar but different env var names", func(t *testing.T) {
		cmd := exec.Command("test")
		cmd.Env = []string{
			"TEST_ENV_NUMBER_EXTRA=foo", // Similar but different
			"MY_PARALLEL_TEST_GROUPS=3", // Similar but different
			"TEST_ENV_NUMBER=1",         // Real one
			"PARALLEL_TEST_GROUPS=2",    // Real one
		}

		result := dryRunString(cmd)

		assert.Contains(t, result, "TEST_ENV_NUMBER=1")
		assert.Contains(t, result, "PARALLEL_TEST_GROUPS=2")
		// Should NOT include the similar-but-different ones
		assert.NotContains(t, result, "TEST_ENV_NUMBER_EXTRA")
		assert.NotContains(t, result, "MY_PARALLEL_TEST_GROUPS")
	})
}
...
func TestBuildEnv(t *testing.T) {
	...
	t.Run("worker indices map correctly with FirstIs1", func(t *testing.T) {
		...
		env0 := runner.buildEnv(0, 3)
		env1 := runner.buildEnv(1, 3)
		env2 := runner.buildEnv(2, 3)

		assertEnvContains(t, env0, "TEST_ENV_NUMBER=1")
		assertEnvContains(t, env1, "TEST_ENV_NUMBER=2")
		assertEnvContains(t, env2, "TEST_ENV_NUMBER=3")
	})

	t.Run("legacy mode (FirstIs1=false) gives empty string for first worker", func(t *testing.T) {
		cfg := &config.GlobalConfig{
			WorkerCount: 3,
			FirstIs1:    false,
		}
		...
		// First worker gets empty string in legacy mode
		assertEnvContains(t, env0, "TEST_ENV_NUMBER=")
		assertEnvContains(t, env1, "TEST_ENV_NUMBER=2")
	})
...
func TestRunnerV2_DryRunReturnsNil(t *testing.T) {
	cfg := &config.GlobalConfig{
		WorkerCount: 2,
		DryRun:      true,
		FirstIs1:    true,
	...
	runner := NewRunnerV2(cfg, []string{"a_test.rb", "b_test.rb"}, testJob)
...
func TestRunnerV2_WorkerCountAdjustment(t *testing.T) {
	t.Run("more workers than files reduces to file count", func(t *testing.T) {
		...
		runner := NewRunnerV2(cfg, files, testJob)
...
func TestRunnerV2_EmptyFiles(t *testing.T) {
	cfg := &config.GlobalConfig{
		WorkerCount: 4,
		DryRun:      true,
		FirstIs1:    true,
	...
	runner := NewRunnerV2(cfg, []string{}, testJob)
	results, wallTime, err := runner.Run()
...
func TestRunnerV2_TrackerInitialized(t *testing.T) {
	cfg := &config.GlobalConfig{
		WorkerCount: 2,
		DryRun:      true,
		FirstIs1:    true,
	...
	runner := NewRunnerV2(cfg, []string{"a_test.rb"}, testJob)
...
func TestRunnerV2_SingleFileStillSetsTestEnvNumber(t *testing.T) {
	...
	runner := NewRunnerV2(cfg, []string{"single_test.rb"}, testJob)
	_, _, err := runner.Run()
...
func TestRunnerV2_SerialModeNoTestEnvNumber(t *testing.T) {
	...
	runner := NewRunnerV2(cfg, []string{"a_test.rb", "b_test.rb", "c_test.rb"}, testJob)
...
func TestRunnerV2_GroupCountMatchesActualGroups(t *testing.T) {
