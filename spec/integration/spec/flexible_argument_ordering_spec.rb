...
RSpec.describe "Flexible argument ordering" do
  context "with --color=never flag" do
    it "works with --color=never before file arguments" do
      Dir.chdir(project_fixture("default-ruby")) do
        result = run_plur("--color=never", "--dry-run", "spec/calculator_spec.rb")
    ...
    it "works with --color=never after file arguments" do
      Dir.chdir(project_fixture("default-ruby")) do
        result = run_plur("spec/calculator_spec.rb", "--color=never", "--dry-run")
  ...
  context "with directory arguments" do
    ...
    it "expands spec/ directory with --color=never after" do
      Dir.chdir(project_fixture("default-ruby")) do
        result = run_plur("spec/", "--color=never", "--dry-run")
  ...
  context "with multiple flags and arguments" do
    it "handles complex argument ordering with flags after files" do
      Dir.chdir(project_fixture("default-ruby")) do
        result = run_plur("spec/calculator_spec.rb", "spec/counter_spec.rb", "--color=never", "-n", "2", "--dry-run")
    ...
    it "handles complex argument ordering with flags before files" do
      Dir.chdir(project_fixture("default-ruby")) do
        result = run_plur("--color=never", "-n", "2", "--dry-run", "spec/calculator_spec.rb", "spec/counter_spec.rb")
