...
output = "auto"          # progress markers on a terminal, summary output otherwise (the default)
...
* `output` - What to print while tests run: `"auto"` (default — progress markers on a terminal, summary output when piped), `"progress"`, or `"summary"`. See [Output Formats](usage.md#output-formats)
...
* `PLUR_OUTPUT` - Output mode from the environment: `auto`, `progress`, or `summary` (same values as `--output`)
